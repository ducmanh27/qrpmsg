...
bool QRPMsg::open(OpenMode mode)
{
    ...
    qDebug() << "Opening with mode:" << mode;
    qDebug() << "  ReadOnly:" << (mode & QIODevice::ReadOnly);
    qDebug() << "  WriteOnly:" << (mode & QIODevice::WriteOnly);
    qDebug() << "  ReadWrite:" << (mode & QIODevice::ReadWrite);
#endif
