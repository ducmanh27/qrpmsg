...
#include <QtRPMsg//QRPMsg>
#include <QtRPMsg/QRPMsgInfo>
...
class tst_QRPMsg : public QObject
{
    Q_OBJECT
public:
    explicit tst_QRPMsg();
...
private:
    QString m_senderChannel;
    QString m_receiverChannel;
...
int tst_QRPMsg::loopLevel = 0;

const QByteArray tst_QRPMsg::alphabetArray("ABCDEFGHIJKLMNOPQRSTUVWXUZ");
const QByteArray tst_QRPMsg::newlineArray("\n\r");

tst_QRPMsg::tst_QRPMsg()
{
    qRegisterMetaType<QRPMsg::RPMsgError>("QRPMsg::RPMsgError");
}

void tst_QRPMsg::initTestCase()
{
    m_senderChannel = QString::fromLocal8Bit(qgetenv("QTEST_RPMSG_SENDER"));
    m_receiverChannel = QString::fromLocal8Bit(qgetenv("QTEST_RPMSG_RECEIVER"));
    if (m_senderChannel.isEmpty() || m_receiverChannel.isEmpty()) {
        static const char message[] =
              "Test doesn't work because the names of rpmsg channel aren't found in env.\n"
              "Please set environment variables:\n"
              " QTEST_RPMSG_SENDER to name of output rpmsg\n"
              " QTEST_RPMSG_RECEIVER to name of input rpmsg\n"
              "Specify short names of channel"
              ", like: rpmsg-openamp-demo-channel\n";

        QSKIP(message);
    } else {
        m_availableChannelNames << m_senderChannel << m_receiverChannel;
    }
}

void tst_QRPMsg::defaultConstruct()
{
    QRPMsg rpmsg;

    QCOMPARE(rpmsg.error(), QRPMsg::NoError);
    QVERIFY(!rpmsg.errorString().isEmpty());

    // QIODevice
    QCOMPARE(rpmsg.openMode(), QIODevice::NotOpen);
    QVERIFY(!rpmsg.isOpen());
    QVERIFY(!rpmsg.isReadable());
    QVERIFY(!rpmsg.isWritable());
    QVERIFY(!rpmsg.isSequential());
    // QCOMPARE(rpmsg.canReadLine(), false);
    QCOMPARE(rpmsg.pos(), qlonglong(0));
    QCOMPARE(rpmsg.size(), qlonglong(0));
    QVERIFY(rpmsg.atEnd());
    QCOMPARE(rpmsg.bytesAvailable(), qlonglong(0));
    QCOMPARE(rpmsg.bytesToWrite(), qlonglong(0));

    char c;
    QCOMPARE(rpmsg.read(&c, 1), qlonglong(-1));
    QCOMPARE(rpmsg.write(&c, 1), qlonglong(-1));
}

void tst_QRPMsg::constructByName()
{
    QRPMsg rpmsg(m_senderChannel);
    QCOMPARE(rpmsg.channelName(), m_senderChannel);
    rpmsg.setChannelName(m_receiverChannel);
    QCOMPARE(rpmsg.channelName(), m_receiverChannel);
}

void tst_QRPMsg::constructByInfo()
{
    QRPMsgInfo senderChannelInfo(m_senderChannel);
    QRPMsgInfo receiverchannelInfo(m_receiverChannel);

#if defined(Q_OS_UNIX)
    if (senderChannelInfo.isNull() || receiverchannelInfo.isNull()) {
        static const char message[] =
                "Test doesn't work because the specified rpmsg channels aren't"
                " found in system and can't be constructed by QRPMsgInfo.\n";
        QSKIP(message);
    }
#endif

    QRPMsg rpmsg(senderChannelInfo);
    QCOMPARE(rpmsg.channelName(), m_senderChannel);
    rpmsg.setChannelName(m_receiverChannel);
    QCOMPARE(rpmsg.channelName(), m_receiverChannel);
}

void tst_QRPMsg::openExisting_data()
{
    QTest::addColumn<int>("openMode");
    QTest::addColumn<bool>("openResult");
    QTest::addColumn<QRPMsg::RPMsgError>("errorCode");

    QTest::newRow("NotOpen") << int(QIODevice::NotOpen) << false << QRPMsg::UnsupportedOperationError;
    QTest::newRow("ReadOnly") << int(QIODevice::ReadOnly) << true << QRPMsg::NoError;
    QTest::newRow("WriteOnly") << int(QIODevice::WriteOnly) << true << QRPMsg::NoError;
    QTest::newRow("ReadWrite") << int(QIODevice::ReadWrite) << true << QRPMsg::NoError;
    QTest::newRow("Append") << int(QIODevice::Append) << false << QRPMsg::UnsupportedOperationError;
    QTest::newRow("Truncate") << int(QIODevice::Truncate) << false << QRPMsg::UnsupportedOperationError;
    QTest::newRow("Text") << int(QIODevice::Text) << false << QRPMsg::UnsupportedOperationError;
    QTest::newRow("Unbuffered") << int(QIODevice::Unbuffered) << false << QRPMsg::UnsupportedOperationError;
}

void tst_QRPMsg::openExisting()
{
    QFETCH(int, openMode);
    QFETCH(bool, openResult);
    QFETCH(QRPMsg::RPMsgError, errorCode);

    for (const QString &channelName : qAsConst(m_availableChannelNames)) {
        QRPMsg rpmsg(channelName);
        QSignalSpy errorSpy(&rpmsg, &QRPMsg::errorOccurred);
        QVERIFY(errorSpy.isValid());

        QCOMPARE(rpmsg.channelName(), channelName);
        QCOMPARE(rpmsg.open(QIODevice::OpenMode(openMode)), openResult);
        QCOMPARE(rpmsg.isOpen(), openResult);
        QCOMPARE(rpmsg.error(), errorCode);

        QCOMPARE(errorSpy.count(), 1);
        QCOMPARE(qvariant_cast<QRPMsg::RPMsgError>(errorSpy.at(0).at(0)), errorCode);
    }
}

void tst_QRPMsg::openNotExisting_data()
{
    QTest::addColumn<QString>("serialPortName");
    QTest::addColumn<bool>("openResult");
    QTest::addColumn<QRPMsg::RPMsgError>("errorCode");

    QTest::newRow("Empty") << QString("") << false << QRPMsg::DeviceNotFoundError;
    QTest::newRow("Null") << QString() << false << QRPMsg::DeviceNotFoundError;
    QTest::newRow("NotExists") << QString("ABCDEF") << false << QRPMsg::DeviceNotFoundError;
}

// void tst_QRPMsg::openNotExisting()
// {
//     QFETCH(QString, serialPortName);
//     QFETCH(bool, openResult);
//     //QFETCH(QRPMsg::SerialPortError, errorCode);

//     QRPMsg serialPort(serialPortName);

//     QSignalSpy errorSpy(&serialPort, &QRPMsg::errorOccurred);
//     QVERIFY(errorSpy.isValid());
...
//     //QCOMPARE(errorSpy.count(), 1);
//     //QCOMPARE(qvariant_cast<QRPMsg::SerialPortError>(errorSpy.at(0).at(0)), errorCode);
// }

// void tst_QRPMsg::baudRate_data()
// {
//     QTest::addColumn<qint32>("baudrate");
//     QTest::newRow("Baud1200") << static_cast<qint32>(QRPMsg::Baud1200);
//     QTest::newRow("Baud2400") << static_cast<qint32>(QRPMsg::Baud2400);
//     QTest::newRow("Baud4800") << static_cast<qint32>(QRPMsg::Baud4800);
//     QTest::newRow("Baud9600") << static_cast<qint32>(QRPMsg::Baud9600);
//     QTest::newRow("Baud19200") << static_cast<qint32>(QRPMsg::Baud19200);
//     QTest::newRow("Baud38400") << static_cast<qint32>(QRPMsg::Baud38400);
//     QTest::newRow("Baud57600") << static_cast<qint32>(QRPMsg::Baud57600);
//     QTest::newRow("Baud115200") << static_cast<qint32>(QRPMsg::Baud115200);

//     QTest::newRow("31250") << 31250; // custom baudrate (MIDI)
// }

// void tst_QRPMsg::baudRate()
// {
//     QFETCH(qint32, baudrate);

//     {
//         // setup before opening
//         QRPMsg serialPort(m_senderPortName);
...
// void tst_QRPMsg::dataBits_data()
// {
//     QTest::addColumn<QRPMsg::DataBits>("databits");
//     QTest::newRow("Data5") << QRPMsg::Data5;
//     QTest::newRow("Data6") << QRPMsg::Data6;
//     QTest::newRow("Data7") << QRPMsg::Data7;
//     QTest::newRow("Data8") << QRPMsg::Data8;
// }

// void tst_QRPMsg::dataBits()
// {
//     QFETCH(QRPMsg::DataBits, databits);
...
// void tst_QRPMsg::parity_data()
// {
//     QTest::addColumn<QRPMsg::Parity>("parity");
//     QTest::newRow("NoParity") << QRPMsg::NoParity;
//     QTest::newRow("EvenParity") << QRPMsg::EvenParity;
//     QTest::newRow("OddParity") << QRPMsg::OddParity;
//     QTest::newRow("SpaceParity") << QRPMsg::SpaceParity;
//     QTest::newRow("MarkParity") << QRPMsg::MarkParity;
// }

// void tst_QRPMsg::parity()
// {
//     QFETCH(QRPMsg::Parity, parity);
...
// void tst_QRPMsg::stopBits_data()
// {
//     QTest::addColumn<QRPMsg::StopBits>("stopbits");
//     QTest::newRow("OneStop") << QRPMsg::OneStop;
// #ifdef Q_OS_WIN
//     QTest::newRow("OneAndHalfStop") << QRPMsg::OneAndHalfStop;
// #endif
//     QTest::newRow("TwoStop") << QRPMsg::TwoStop;
// }

// void tst_QRPMsg::stopBits()
// {
//     QFETCH(QRPMsg::StopBits, stopbits);
...
// void tst_QRPMsg::flowControl_data()
// {
//     QTest::addColumn<QRPMsg::FlowControl>("flowcontrol");
//     QTest::newRow("NoFlowControl") << QRPMsg::NoFlowControl;
//     QTest::newRow("HardwareControl") << QRPMsg::HardwareControl;
//     QTest::newRow("SoftwareControl") << QRPMsg::SoftwareControl;
// }

// void tst_QRPMsg::flowControl()
// {
//     QFETCH(QRPMsg::FlowControl, flowcontrol);
...
//     QSignalSpy errorSpy(&serialPort, &QRPMsg::errorOccurred);
//     QVERIFY(errorSpy.isValid());
//     QSignalSpy rtsSpy(&serialPort, &QRPMsg::requestToSendChanged);
//     QVERIFY(rtsSpy.isValid());

//     QVERIFY(serialPort.open(QIODevice::ReadWrite));

//     // no flow control
//     QVERIFY(serialPort.setFlowControl(QRPMsg::NoFlowControl));
...
//     QVERIFY(serialPort.setFlowControl(QRPMsg::SoftwareControl));
...
//     QVERIFY(serialPort.setFlowControl(QRPMsg::HardwareControl));
...
//     QCOMPARE(serialPort.error(), QRPMsg::UnsupportedOperationError);

//     QCOMPARE(errorSpy.count(), 2);
//     QCOMPARE(qvariant_cast<QRPMsg::SerialPortError>(errorSpy.at(0).at(0)), QRPMsg::NoError);
//     QCOMPARE(qvariant_cast<QRPMsg::SerialPortError>(errorSpy.at(1).at(0)), QRPMsg::UnsupportedOperationError);
...
//     QSignalSpy errorSpy(&serialPort, &QRPMsg::errorOccurred);
//     QVERIFY(errorSpy.isValid());
//     QSignalSpy dtrSpy(&serialPort, &QRPMsg::dataTerminalReadyChanged);
...
// void tst_QRPMsg::independenceRtsAndDtr()
// {
//     QRPMsg serialPort(m_senderPortName);
...
//     QVERIFY(serialPort.setDataBits(QRPMsg::Data7));
//     QVERIFY(serialPort.isRequestToSend());
//     QVERIFY(serialPort.isDataTerminalReady());

//     // check that parity changing does not change DTR or RTS
//     QVERIFY(serialPort.setParity(QRPMsg::EvenParity));
//     QVERIFY(serialPort.isRequestToSend());
//     QVERIFY(serialPort.isDataTerminalReady());

//     // check that stop bits changing does not change DTR or RTS
//     QVERIFY(serialPort.setStopBits(QRPMsg::TwoStop));
//     QVERIFY(serialPort.isRequestToSend());
//     QVERIFY(serialPort.isDataTerminalReady());
...
// void tst_QRPMsg::handleBytesWrittenAndExitLoopSlot(qint64 bytesWritten)
// {
//     QCOMPARE(bytesWritten, qint64(alphabetArray.size() + newlineArray.size()));
//     exitLoop();
// }

// void tst_QRPMsg::flush()
// {
// #ifdef Q_OS_WIN
...
//     QRPMsg dummySerialPort(m_receiverPortName);
//     QVERIFY(dummySerialPort.open(QIODevice::ReadOnly));

//     QRPMsg serialPort(m_senderPortName);
//     connect(&serialPort, &QRPMsg::bytesWritten, this, &tst_QRPMsg::handleBytesWrittenAndExitLoopSlot);
//     QSignalSpy bytesWrittenSpy(&serialPort, &QRPMsg::bytesWritten);
...
// void tst_QRPMsg::handleBytesWrittenAndExitLoopSlot2(qint64 bytesWritten)
// {
...
//     QRPMsg serialPort(m_senderPortName);
//     connect(&serialPort, &QRPMsg::bytesWritten, this, &tst_QRPMsg::handleBytesWrittenAndExitLoopSlot2);
//     QSignalSpy bytesWrittenSpy(&serialPort, &QRPMsg::bytesWritten);
...
// void tst_QRPMsg::waitForBytesWritten()
// {
//     // the dummy device on other side also has to be open
//     QRPMsg dummySerialPort(m_receiverPortName);
//     QVERIFY(dummySerialPort.open(QIODevice::ReadOnly));
...
// void tst_QRPMsg::waitForReadyReadWithTimeout()
// {
//     // the dummy device on other side also has to be open
//     QRPMsg dummySerialPort(m_senderPortName);
//     QVERIFY(dummySerialPort.open(QIODevice::WriteOnly));

//     QRPMsg receiverSerialPort(m_receiverPortName);
...
//     QCOMPARE(receiverSerialPort.error(), QRPMsg::TimeoutError);
// }

// void tst_QRPMsg::waitForReadyReadWithOneByte()
// {
//     const qint64 oneByte = 1;
//     const int waitMsecs = 50;

//     QRPMsg senderSerialPort(m_senderPortName);
//     QVERIFY(senderSerialPort.open(QIODevice::WriteOnly));
//     QRPMsg receiverSerialPort(m_receiverPortName);
//     QSignalSpy readyReadSpy(&receiverSerialPort, &QRPMsg::readyRead);
...
//     QCOMPARE(receiverSerialPort.error(), QRPMsg::NoError);
//     QCOMPARE(readyReadSpy.count(), 1);
// }

// void tst_QRPMsg::waitForReadyReadWithAlphabet()
// {
//     const int waitMsecs = 50;
...
// void tst_QRPMsg::twoStageSynchronousLoopback()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::ReadWrite));

//     QRPMsg receiverPort(m_receiverPortName);
//     QVERIFY(receiverPort.open(QRPMsg::ReadWrite));
...
// void tst_QRPMsg::synchronousReadWrite()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::WriteOnly));

//     QRPMsg receiverPort(m_receiverPortName);
//     QVERIFY(receiverPort.open(QRPMsg::ReadOnly));
...
//     Q_OBJECT
// public:
//     explicit AsyncReader(QRPMsg &port, Qt::ConnectionType connectionType, int expectedBytesCount)
//         : serialPort(port), expectedBytesCount(expectedBytesCount)
//     {
//         connect(&serialPort, &QRPMsg::readyRead, this, &AsyncReader::receive, connectionType);
//     }
...
//             QRPMsg &port, Qt::ConnectionType connectionType, const QByteArray &dataToWrite)
//         : serialPort(port), writeChunkSize(0)
//     {
//         writeBuffer.setData(dataToWrite);
//         writeBuffer.open(QIODevice::ReadOnly);
//         connect(&serialPort, &QRPMsg::bytesWritten, this, &AsyncWriterByBytesWritten::send, connectionType);
//         send();
//     }
...
// void tst_QRPMsg::asynchronousWriteByBytesWritten_data()
// {
...
// void tst_QRPMsg::asynchronousWriteByBytesWritten()
// {
...
// void tst_QRPMsg::asynchronousWriteByTimer_data()
// {
//     QTest::addColumn<Qt::ConnectionType>("readConnectionType");
//     QTest::addColumn<Qt::ConnectionType>("writeConnectionType");
...
// void tst_QRPMsg::asynchronousWriteByTimer()
// {
//     QFETCH(Qt::ConnectionType, readConnectionType);
//     QFETCH(Qt::ConnectionType, writeConnectionType);
...
//     Q_OBJECT
// public:
//     explicit AsyncReader2(QRPMsg &port, const QByteArray &expectedData)
//         : serialPort(port), expectedData(expectedData)
//     {
//         connect(&serialPort, &QRPMsg::readyRead, this, &AsyncReader2::receive);
//     }
...
// void tst_QRPMsg::asyncReadWithLimitedReadBufferSize()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::WriteOnly));
...
// void tst_QRPMsg::readBufferOverflow()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::WriteOnly));
...
// void tst_QRPMsg::readAfterInputClear()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::WriteOnly));
...
//     QVERIFY(receiverPort.clear(QRPMsg::Input));
...
//     void open()
//     {
//         if (serialPort.open(QRPMsg::ReadWrite)) {
...
//     {
//         connect(&serialPort, &QRPMsg::readyRead, this, &MasterTransactor::transaction);
//     }
...
//         : serialPort(new QRPMsg(name, this))
//     {
//         connect(serialPort, &QRPMsg::readyRead, this, &SlaveTransactor::transaction);
//     }
...
// void tst_QRPMsg::synchronousReadWriteAfterAsynchronousReadWrite()
// {
...
//     tst_QRPMsg::enterLoopMsecs(2000);
...
//     Q_OBJECT
// public:
//     explicit BreakReader(QRPMsg &port)
//         : serialPort(port)
//     {
//         connect(&serialPort, &QRPMsg::readyRead, this, &BreakReader::receive);
//     }
...
// void tst_QRPMsg::controlBreak()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::WriteOnly));
//     QCOMPARE(senderPort.isBreakEnabled(), false);

//     QSignalSpy breakSpy(&senderPort, &QRPMsg::breakEnabledChanged);
...
// void tst_QRPMsg::clearAfterOpen()
// {
//     QRPMsg senderPort(m_senderPortName);
//     QVERIFY(senderPort.open(QRPMsg::ReadWrite));
//     QCOMPARE(senderPort.error(), QRPMsg::NoError);
//     QVERIFY(senderPort.clear());
//     QCOMPARE(senderPort.error(), QRPMsg::NoError);
// }

// void tst_QRPMsg::readWriteWithDifferentBaudRate_data()
// {
...
// void tst_QRPMsg::readWriteWithDifferentBaudRate()
// {
